...
<body>
	<?php
		...
		if($_SESSION['matricola']){
			...
			if($commento){
				$query='INSERT INTO Commenti_Utente (IdFeedback,commentatore,testo) VALUES ("'.$feedback.'","'.$matricola.'","'.$commento.'")';
				$result = mysql_query($query);
			}
			$query="SELECT IdCommento,commentatore,testo,N_piace,N_nopiace FROM Commenti_Utente WHERE IdFeedback=".$feedback." ORDER BY data";
			$result = mysql_query($query);
			...
			while($resultset = mysql_fetch_array($result)){
				...
				print('<DIV class="cella"> <A href="piacefeedback.php?id='.$resultset[0].'&piace=y&feed='.$feedback.'"><img src="../images/piace.jpg" width="15" height="15"></A> '.$resultset[3].' <A href="piacefeedback.php?id='.$resultset[0].'&feed='.$feedback.'"><img src="../images/nopiace.jpg" width="15" height="15"></A> '.$resultset[4].'</DIV>');
				print('</DIV><br>');
			}
			print("</DIV>");
			
			print('<B>Inserisci Un Commento</B>');
			print('<form  name="comment" action="commentifeedback.php?feedback='.$feedback.'" method="post">');
